...
import { Map, Layers, Calendar, Clock, TrendingUp, AlertCircle, CheckCircle, Info } from 'lucide-react';
...
interface ContentLandscapeData {
    ...
    formats: Array<{ name: string; count: number; icon: string }> | null;
    uploadConsistency: {
        score: number;
    ...
    hasRealFormatData?: boolean;
    hasRealConsistencyData?: boolean;
}
...
export function ContentLandscapeSection({ data }: ContentLandscapeSectionProps) {
    ...
    const consistency = data.uploadConsistency ? getConsistencyLabel(data.uploadConsistency.score) : null;
    ...
    return (
        <div className="space-y-6">
            ...
            {/* Top Stats - Only show what's available */}
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                {/* Topics - always available from title analysis */}
                <div className="bg-white rounded-2xl p-4 border border-slate-100 shadow-sm">
                    <div className="flex items-center gap-2 mb-2">
                        <Map className="w-4 h-4 text-blue-500" />
                        <span className="text-xs text-slate-500 font-medium">Topics Detected</span>
                    </div>
                    <div className="text-2xl font-bold text-slate-900">{data.totalTopics}</div>
                    <div className="text-xs text-slate-400">from video titles</div>
                </div>

                {/* Format Diversity - only if real data */}
                ...
                {/* Upload Consistency - only if real data */}
                {data.uploadConsistency && consistency && (
                    <div className="bg-white rounded-2xl p-4 border border-slate-100 shadow-sm">
                        <div className="flex items-center gap-2 mb-2">
                ...
                <div className="bg-white rounded-[24px] p-6 border border-slate-100 shadow-sm">
                    ...
                    {data.topics.length > 0 ? (
                        ...
                        <div className="flex items-center gap-3 text-slate-500 py-8">
                            ...
                            <span>Not enough video data to analyze topics</span>
                        </div>
                    )}
                </div>
            ...
            {/* Growth Impact Note - only show if we have consistency data */}
            {data.uploadConsistency && (
                <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-2xl p-4 border border-green-100">
                    <div className="flex items-start gap-3">
                        <TrendingUp className="w-5 h-5 text-green-600 mt-0.5" />
                        <div>
                            <h4 className="font-medium text-green-800">Upload Consistency</h4>
                            <p className="text-sm text-green-700 mt-1">
                                Your current upload frequency is approximately every {data.uploadConsistency.avgDaysBetween} days.
                                {data.uploadConsistency.score >= 70
                                    ? " Your consistency is strong — keep it up!"
                                    : " Consider establishing a more regular schedule for better audience retention."}
                            </p>
                        </div>
                    </div>
                </div>
