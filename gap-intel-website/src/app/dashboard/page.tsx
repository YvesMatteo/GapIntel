                                                        ...
                                                        <div className="relative flex-shrink-0">
                                                            <motion.div whileHover={{ scale: 1.1 }} className="relative z-10">
                                                                {report.channel_thumbnail ? (
                                                                    ...
                                                                    <div className="w-10 h-10 md:w-14 md:h-14 rounded-full bg-gradient-to-br from-slate-100 to-slate-200 border-2 border-white shadow-md flex items-center justify-center text-slate-400 text-base md:text-lg font-bold">
                                                                        {(report.channel_name || 'C')[0].toUpperCase()}
                                                                    </div>
                                                                )}
                                                            </motion.div>
                                                            <div className="absolute -bottom-1 -right-1 w-5 h-5 md:w-6 md:h-6 rounded-full bg-white flex items-center justify-center shadow-sm z-20">
                                                                {getStatusIcon(report.status)}
                                                            </div>
                                                        </div>

                                                        <div className="min-w-0 flex-1">
                                                            <h4 className="text-base md:text-lg font-semibold text-slate-900 mb-1 truncate">
                                                                {report.channel_name}
                                                                <span className="text-slate-400 font-normal text-xs md:text-sm ml-1 md:ml-2">@{report.channel_handle}</span>
                                                            </h4>
                                                            <div className="flex flex-wrap items-center gap-2 md:gap-3 text-xs md:text-sm text-slate-500">
                                                                <span className={`px-2 py-0.5 rounded-full text-xs font-bold border ${report.status === 'completed' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' :
                                                    ...
                                                    <div className="flex items-center gap-2 md:gap-3 ml-auto md:ml-0 opacity-100">
                                                        ...
                                                        <div ref={showFolderMenu === report.id ? folderMenuRef : undefined} className="relative">
                                                            <button
                                                                ...
                                                                className="p-2 md:p-2.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg md:rounded-xl transition-all"
                                                        ...
                                                        {report.status === "completed" && (
                                                            <Link href={`/report/${report.access_key}`}>
                                                                <motion.button
                                                                    whileHover={{ scale: 1.05 }}
                                                                    whileTap={{ scale: 0.95 }}
                                                                    className="h-8 md:h-10 px-3 md:px-6 bg-slate-900 md:bg-white md:border md:border-slate-200 text-white md:text-slate-900 rounded-lg md:rounded-full text-xs md:text-sm font-bold md:hover:bg-slate-50 md:hover:border-slate-300 transition-all shadow-sm flex items-center gap-1 md:gap-2"
                                                                >
