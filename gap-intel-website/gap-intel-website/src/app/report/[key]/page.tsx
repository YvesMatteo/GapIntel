...
import { CompetitorComparisonSection } from "@/components/report/CompetitorComparisonSection";
                    ...
                    {(() => {
                        ...
                        return (
                            <>
                                ...
                                {analysis.report_data?.premium?.competitor_intel?.competitors && analysis.report_data.premium.competitor_intel.competitors.length > 0 && (
                                    <>
                                        <div className="flex items-center gap-4 pt-8">
                                            <div className="h-px bg-gradient-to-r from-transparent via-slate-200 to-transparent flex-1" />
                                            <h2 className="text-sm font-bold text-slate-400 uppercase tracking-widest">Competitor Comparison</h2>
                                            <div className="h-px bg-gradient-to-r from-transparent via-slate-200 to-transparent flex-1" />
                                        </div>
                                        <CompetitorComparisonSection
                                            userChannel={{
                                                name: analysis.channel_name,
                                                avgViews: Math.round(analysis.report_data?.videos_analyzed?.reduce((sum, v) => sum + (v as any).view_count || 0, 0) / (analysis.report_data?.videos_analyzed?.length || 1)) || 0,
                                                cvr: engagementMetrics.cvr,
                                                formatMix: contentLandscape.formats?.[0]?.name || 'Mixed',
                                                shortsCount: contentLandscape.formats?.find(f => f.name.toLowerCase().includes('short'))?.count || 0,
                                                longFormCount: contentLandscape.formats?.find(f => !f.name.toLowerCase().includes('short'))?.count || analysis.report_data?.videos_analyzed?.length || 0,
                                            }}
                                            competitors={analysis.report_data.premium.competitor_intel.competitors.map(c => ({
                                                name: c.channel_name,
                                                subscribers: c.subscriber_count,
                                                avgViews: c.avg_views,
                                                cvr: c.avg_views / Math.max(c.subscriber_count, 1) * 100,
                                                formatMix: c.top_formats?.[0] || 'Mixed',
                                                shortsCount: c.top_formats?.filter(f => f.toLowerCase().includes('short')).length || 0,
                                                longFormCount: c.top_formats?.filter(f => !f.toLowerCase().includes('short')).length || 1,
                                            }))}
                                        />
                                    </>
                                )}
